			...
			unsigned long arg)
{
  ...
  switch (ioctl_num)
  {
    ...
    case LCD_IOCTL_RESET:
      get_user(ch, buffer);
      if (ch == '1') lcdinit(data, data->organization.topology);
      break;

    case LCD_IOCTL_HOME:
      printk(KERN_INFO "HOME called\n");
      get_user(ch, buffer);
      if (ch == '1') lcdhome(data);
      break;
    ...
    case LCD_IOCTL_GETCUSTOMCHAR:
      get_user(ch, buffer);
      for (i = 0; i < 8; i++) put_user(data->customchars[ch][i], buffer + i + 1);
      break;

    case LCD_IOCTL_SETCUSTOMCHAR:
      for (i = 0; i < 9; i++) get_user(ccb[i], buffer + i);
      lcdcustomchar(data, ccb[0], ccb+1);
      break;

    case LCD_IOCTL_CLEAR:
      get_user(ch, buffer);
      if (ch == '1') lcdclear(data);
      break;
				...
				const char* buf, size_t count)
{
    ...
    if (er == 0)
    {
        ...
        dev_err(dev, "Brightness argument is out of range 0-255");
    else if (er == -EINVAL)
        dev_err(dev, "Brightness argument has no numerical representation \"%s\"", buf);
    else
        dev_err(dev, "Brightness parameter wasn't properly converted to numeric type %d", er);
